#> pk_racks:blocks/rack/use/ground/change_item/2/swap
...
data remove entity @e[type=item_display,tag=pk.temp.current.component,tag=pk.item.2,distance=..18,limit=1] item
...
# Logs
execute if score $logs.datapack.racks pk.value matches 1 run tellraw @a[tag=pk.dev] [{"selector":"@a[tag=pk.temp.current.player,limit=1]","color": "gray"},{"text": " swapped Rack's item (2) at ["},{"nbt":"Temp.Rack.x","storage":"pk.common:data"},{"text": ","},{"nbt":"Temp.Rack.y","storage":"pk.common:data"},{"text": ","},{"nbt":"Temp.Rack.z","storage":"pk.common:data"},{"text": "]"}]