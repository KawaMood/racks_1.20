#> pk_racks:blocks/rack/place/blocks_list/set_dimension_id

# Default score
scoreboard players set $temp pk.dimension.id -99

# If placed in a vanilla dimension
execute if predicate pk_racks:location/dimension/0 run scoreboard players set $temp pk.dimension.id 0
execute if predicate pk_racks:location/dimension/-1 run scoreboard players set $temp pk.dimension.id -1
execute if predicate pk_racks:location/dimension/1 run scoreboard players set $temp pk.dimension.id 1
