#> pk_racks:blocks/rack/remove/drop_content/1

# Prepare item
summon item ~ ~ ~ {Item:{id:"minecraft:stick",Count:1b},Tags:["pk.temp.current.item"]}
data modify entity @e[type=item,tag=pk.temp.current.item,distance=..0.1,limit=1] Item set from storage pk.common:data Temp.Rack.Items[0]
